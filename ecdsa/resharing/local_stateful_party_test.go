...
resharingFirstPart:
	...
	for _, P := range oldCommittee {
		go func(P *LocalStatefulParty) {
			mutexOld.Lock()
			if errR := P.Restart(TaskName, 3, stateOldCommittee[*P.PartyID()], sessionId); errR != nil {
				errCh1 <- errR
			}
			mutexOld.Unlock()
		}(P)
	...
	for _, P := range newCommittee {
		go func(P *LocalStatefulParty) {
			mutexNew.Lock()
			if errR := P.Restart(TaskName, 3, stateNewCommittee[*P.PartyID()], sessionId); errR != nil {
				errCh2 <- errR
			}
			mutexNew.Unlock()
		}(P)
