...
protob:
	...
	@for protocol in shared message-data end-data ecdsa-keygen ecdsa-signing ecdsa-resharing schnorr-keygen schnorr-signing schnorr-resharing; do \
		echo "Generating $$protocol.pb.go" ; \
		protoc --proto_path=./protob --go_out=. ./protob/$$protocol.proto ; \
	done
