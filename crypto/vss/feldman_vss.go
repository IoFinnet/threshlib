...
func Create(ec elliptic.Curve, threshold int, secret *big.Int, indexes []*big.Int) (Vs, Shares, error) {
	...
	wg1.Add(len(poly))
	for i, ai := range poly {
		go func(i int, ai *big.Int) {
			defer wg1.Done()
			var errV error
			if v[i], errV = crypto.ScalarBaseMult(ec, ai); errV != nil {
				err1 = errV
				return
			}
		}(i, ai)
	...
	for i := 0; i < num; i++ {
		go func(i int) {
			...
			if indexes[i].Cmp(big.NewInt(0)) == 0 {
				err2 = fmt.Errorf("party index should not be 0")
				return
			}
...
func (share *Share) Verify(ec elliptic.Curve, threshold int, vs Vs) bool {
	...
	var sigmaGi *crypto.ECPoint
	wg := sync.WaitGroup{}
	wg.Add(1)
	go func() {
		defer wg.Done()
		sigmaGi, _ = crypto.ScalarBaseMult(ec, share.Share)
	}()
	...
	for j := 1; j <= threshold; j++ {
		...
		// v = v * v_j^t
		vjt, err := vs[j].SetCurve(ec).ScalarMult(t)
		if err != nil {
			return false
		}
		if v, err = v.SetCurve(ec).Add(vjt); err != nil {
			return false
		}
	}
	wg.Wait()
