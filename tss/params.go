...
func NewParameters(ec elliptic.Curve, ctx *PeerContext, partyID *PartyID, partyCount, threshold int, optionalSafePrimeGenTimeout ...time.Duration) (*Parameters, error) {
	...
	// Default round operation timeout - 1 minute
	roundCtx := NewRoundContext(1 * time.Minute)
	params := &Parameters{
		ec:                  ec,
		parties:             ctx,
		partyID:             partyID,
...
// RoundContext returns the context for managing goroutines in rounds
func (params *Parameters) RoundContext() *RoundContext {
	return params.roundContext
}
