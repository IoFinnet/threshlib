...
func (round *round2) Start() *tss.Error {
	...
	// 5. compute BIP-340 prove
	if round.temp.sessionId == nil {
		return round.WrapError(errors.New("sessionId not set"))
	}
	pii, err := zkpsch.NewProofWithNonce(round.temp.vs[0], round.temp.ui, round.temp.sessionId)
	if err != nil {
		return round.WrapError(errors2.Wrapf(err, "NewZKProof(ui, vi0)"))
	}

	// 5. BROADCAST de-commitments of Shamir poly*G and BIP-340 prove
