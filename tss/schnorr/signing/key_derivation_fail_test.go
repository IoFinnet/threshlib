...
func TestMismatchedKeyDerivationDelta(t *testing.T) {
	...
	for i := range deltas {
		...
		k := keys[i].Copy()
		keyCopies[0] = &k
		err = UpdatePublicKeyAndAdjustBigXj(deltas[i], keyCopies, nil, tss.Edwards())
	...
	msg := []byte("test message for mismatched deltas")
...
signing:
	for {
		...
		case err := <-errCh:
			...
			if atomic.LoadInt32(&errored) > 0 {
				...
				if strings.Contains(strings.ToLower(errMsg), "fail") || strings.Contains(errMsg, "inconsistent") {
	...
	t.Logf("Test successful - the EDDSA signing protocol failed as expected with mismatched deltas")
}
