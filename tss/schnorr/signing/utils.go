...
func BIP340Verify(p *btcec.PublicKey, m []byte, r *big.Int, s *big.Int) error {
	var R btcec.FieldVal
	R.SetByteSlice(r.Bytes())
	var S btcec.ModNScalar
	S.SetByteSlice(s.Bytes())
	return bip340Verify(m, p, R, S)
}
...
func bip340Verify(hash []byte, pubKey *btcec.PublicKey, r btcec.FieldVal, s btcec.ModNScalar) error {
	...
	if overflow := e.SetBytes((*[32]byte)(commitment.CloneBytes())); overflow != 0 {
		str := "hash of (r || P || m) too big"
		return signatureError("ErrBIP340HashValue", str)
	}
...
// littleEndianBytesToBigInt converts a LE byte array such as the ones used in ed25519 Scalar to big integer
func littleEndianBytesToBigInt(s []byte) *big.Int {
...
func encode32bytes(i *big.Int, buff []byte) []byte {
	return i.FillBytes(buff[:])
}
