...
func (round *round4) Start() *tss.Error {
	...
	wg.Wait()
	if len(culprits) > 0 {
		return round.WrapError(errors.New("EdDSA VSS verification failed"), culprits...)
	}
	...
	for c := 0; c <= round.NewThreshold(); c++ {
		...
		for j := 1; j <= len(vjc)-1; j++ {
			if Vc[c], err = Vc[c].Add(vjc[j][c]); err != nil {
				return round.WrapError(errors.Wrapf(err, "Vc[c].Add(vjc[j][c])"))
			}
		}
	}

	// In the migratory case where we are moving from ECDSA to EdDSA, make sure the public key is valid and updated
	if round.input.EDDSAPub == nil {
		eddsaPubKey, err := crypto.NewECPoint(round.Params().EC(), Vc[0].X(), Vc[0].Y())
		if err != nil {
			return round.WrapError(errors.Wrapf(err, "public key is not on the curve"))
		}
		round.input.EDDSAPub = eddsaPubKey
	}
	round.save.EDDSAPub = round.input.EDDSAPub
